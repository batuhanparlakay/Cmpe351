...
SELECT COUNT(essn) FROM DEPENDENT WHERE essn=333445555;

SELECT COUNT(essn) FROM DEPARTMENT,DEPENDENT WHERE (DEPARTMENT.mgr_ssn = DEPENDENT.essn);

SELECT AVG(salary) FROM EMPLOYEE,DEPENDENT WHERE DEPENDENT.relationship = "Spouse";

SELECT MIN(salary) FROM EMPLOYEE,DEPENDENT WHERE(EMPLOYEE.dnumber=1 AND (DEPENDENT.relationship="Daughter" OR DEPENDENT.relationship="Son") );
